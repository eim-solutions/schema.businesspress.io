...
- The extension makes no automatic network requests.
- Social preview images stay blocked unless you click **Load preview images**. Chrome then requests only the declared image URLs directly, without a referrer. No page content or report is sent to SEOMarkup.
...
The public website at `schema.businesspress.io` also offers a separate source-only inspection mode. It sends one user-submitted public URL to the BusinessPress server over POST, fetches up to 2 MB of source HTML, and returns the page to the active browser tab for analysis. The application creates no account, scan-history record, or saved report. Private/reserved addresses, login details, non-standard ports, oversized pages, and unsafe redirects are blocked. The social-image option is checked by default, so opening the Social tab loads declared images directly from their source without a referrer. If you uncheck it before inspection, images stay blocked until you click **Load preview images**.

Each completed report gets a report-specific share link containing a compressed report after the URL's `#`. Browsers do not send that fragment to the server, and SEOMarkup does not store it. Anyone with the link can read the report. Restoring it does not fetch the inspected page again, and decoding rejects reports that expand beyond 2 MB.
